...
function (angular, $, kbn, _, config, moment, Modernizr) {
  ...
  module.service('dashboard', function(
    ...
    this.file_load = function(file) {
      // Look for file under pre-defined file location internal to Kibana
      var url = "app/dashboards/"+file.replace(/\.(?!js)/g,"/");
      return this.url_load_internal(url);
    };

    this.url_load = function(url) {
      // Look for file based on absolute URL path on this server.  Assume URL path is absolute by prepending a '/'
      return this.url_load_internal('/' + url.replace(/\.(?!js)/g,"/"));
    };

    this.url_load_internal = function(url) {
      return $http({
        ...
        alertSrv.set('Error',"Could not load <i>"+ url +"</i>. Please make sure it exists" ,'error');
        return false;
      });
    };
